...
def set_googlesheetintegration(db):
    ...
    googlesheetintegration = GoogleSheetIntegration.objects.create(user=user, id=poll)
...
@pytest.mark.skip(reason="field became OneToOne")
...
def test_on_delete_user_field(set_googlesheetintegration):
    field_param = set_googlesheetintegration._meta.gget_field('user').on_delete
    assert field_param == models.CASCADE


@pytest.mark.skip(reason="field is delefed")
...
def test_GET_GS_integration_list(set_uris):
    response = requests.post(set_uris['uri_auth'], data={"email": "test@test.com",
                                                         "password": "1"})
