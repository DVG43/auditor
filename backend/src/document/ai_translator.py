...
def theme_to_paragraph_prompt(
    ...
    >>> theme_to_paragraph_preprocess("Искусственный интеллект")
    ...
    if keywords:
        prompt += f'Use keywords: {keywords}\n'
...
def rephrase_prompt(text: str) -> str:
    ...
    >>> rephrase_preprocess("Пора менять карьеру и жизнь")
...
def rephrase_postprocess(generated_text: str) -> list:
    """
    Удалить нумерацию.

    >>> print(rephrase_postprocess('''\
    ...     1. Пришло время менять профессию и жизнь.
    ...     2. Настало время поменять профессию и жизнь.
    ...     3. Пора изменить карьеру и жизнь.
    ... '''))
